<div align="center">

  # 📱 Smart Collect

  **Application mobile professionnelle de collecte de données terrain et synchronisation Google Drive.**

  [![Version](https://img.shields.io/badge/Release-v1.0.0-blue.svg?style=for-the-badge&logo=github)](https://github.com/AlhsGdev/smartcollect_apk/releases/latest)
  [![Android](https://img.shields.io/badge/Android-5.0+-3DDC84.svg?style=for-the-badge&logo=android&logoColor=white)](https://github.com/AlhsGdev/smartcollect_apk/releases/latest)
  [![Build](https://img.shields.io/badge/APK-Offusqué%20(Sécurisé)-success.svg?style=for-the-badge)](https://github.com/AlhsGdev/smartcollect_apk/releases/latest)

  <br /><br />

  <a href="https://github.com/AlhsGdev/smartcollect_apk/releases/download/v1.0.0/app-release.apk">
    <img src="https://img.shields.io/badge/⚡_TÉLÉCHARGER_L'APPLICATION_ANDROID-2563EB?style=for-the-badge&logo=android&logoColor=white" height="52" alt="Télécharger APK" />
  </a>

  <br /><br />
  
  [🌐 Site Officiel](https://smartcollect.online) • [🔒 Confidentialité](https://smartcollect.online/privacy.html) • [📜 Conditions](https://smartcollect.online/terms.html)

</div>

---

### 📋 Présentation

**Smart Collect** simplifie la saisie et la structuration d'informations tabulaires sans nécessiter de connexion Internet. L'application conserve les données localement sur votre terminal et permet la synchronisation sur votre compte Google Drive personnel (`drive.file`) sans aucun serveur tiers.

---

### ⚡ Fonctionnalités

| Module | Description |
| :--- | :--- |
| 📊 **Tableaux personnalisés** | Création de tables avec champs textes, numériques, indicatifs téléphoniques internationaux et menus déroulants. |
| 🏷️ **Gestion de l'obligation** | Préfixez un nom de champ par `@` (ex: `@Email`) pour le rendre facultatif. Tous les autres champs sont requis. |
| 📅 **Suivi & Rendez-vous** | Planification, rappels d'échéances et historique d'appels par entrée. |
| 📞 **Actions directes** | Déclenchement d'appels téléphoniques ou ouverture de conversations WhatsApp en 1 clic. |
| 🖨️ **Impression PDF** | Génération de rapports personnalisés (Portrait / Paysage, réglage de police, filtres sélectifs). |
| 📑 **Compatibilité Excel** | Export de modèles vierges et import direct depuis des fichiers `.xlsx`. |
| 🔒 **Sécurité renforcée** | Binaire compilé avec offuscation Dart (`--obfuscate`) pour protéger le code source. |

---

### 📥 Comment installer l'application ?

1. Cliquez sur le bouton bleu **« TÉLÉCHARGER L'APPLICATION ANDROID »** ci-dessus (ou rendez-vous dans les [Releases](https://github.com/AlhsGdev/smartcollect_apk/releases/latest)).
2. Une fois le fichier `.apk` téléchargé sur votre smartphone, ouvrez-le.
3. Si Android affiche une alerte de sécurité, choisissez **« Paramètres »** puis activez **« Autoriser cette source »** pour lancer l'installation.

---

<div align="center">
  <sub>© 2026 Smart Collect • Développé pour la fiabilité et la souveraineté des données.</sub>
</div>
